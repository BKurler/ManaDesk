/*
 * Contributors:
 *     Rémi Dutil (2026) - created for ManaDesk: proxy filter checkbox group
 */
...
import com.reflexit.magiccards.core.model.Proxies;

public class ProxyPreferenceGroup extends MFieldEditorPreferencePage {
	private Group group;
	...
	protected void createFieldEditors() {
		...
		this.group.setText("Proxy");
		this.group.setFont(getFieldEditorParent().getFont());
		Composite parent = this.group;
		Proxies proxies = Proxies.getInstance();
		for (Iterator iterator = proxies.getIds().iterator(); iterator.hasNext();) {
			String id = (String) iterator.next();
			addCheckBox(id, proxies.getNameById(id), parent);
		}
	}

	public ISearchableProperty getSearchablePropery() {
		return Proxies.getInstance();
	}
