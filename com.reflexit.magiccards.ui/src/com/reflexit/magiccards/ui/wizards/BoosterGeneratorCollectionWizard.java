...
public class BoosterGeneratorCollectionWizard extends NewCardElementWizard implements INewWizard {
	...
	protected void doFinish(final String containerName, final String name, boolean virtual, boolean unsorted,
		...
		CardCollection col = populateLibrary(BoosterGeneratorCollectionWizard.this.sets,
				BoosterGeneratorCollectionWizard.this.packs, name, parent, virtual, unsorted,
	...
	private CardCollection populateLibrary(List collections, int packs, String name, CardOrganizer parent,
		...
		if (!separateCollections) {
			col = new CardCollection(name + ".xml", parent, false, virtual, unsorted);
			try {
				// 1*packs rare cards
			...
			for (int pack = 0; pack < packs; pack++) {
				col = new CardCollection(name + "_" + (pack + 1) + ".xml", parent, false, virtual, false);
				try {
					// 1*packs rare cards
	...
	private boolean prompt(final String str) {
		...
		getShell().getDisplay().syncExec(new Runnable() {
			@Override
			public void run() {
				boolean ok = MessageDialog.openConfirm(getShell(), "Confirm",
						str + ". Press OK to continue generation and Cancel to abort and change selection and options");
				result[0] = ok;
			}
		});
		if (!result[0]) {
	 ...
	 * @param col
	 */
	private void generateRandom(int num, IFilteredCardStore<IMagicCard> dbcards, ICardStore<IMagicCard> store,
			CardElement col) {
		for (int i = 0; i < num; i++) {
			try {
