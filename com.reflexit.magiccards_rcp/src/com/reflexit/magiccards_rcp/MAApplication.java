 ...
 * @since 3.0
 */
public class MAApplication implements IApplication, IExecutableExtension {
	/**
	...
	private Object checkInstanceLocation(Shell shell, Map applicationArguments) {
		...
		Location instanceLoc = Platform.getInstanceLocation();
		if (instanceLoc == null) {
			MessageDialog.openError(shell, MAWorkbenchMessages.IDEApplication_workspaceMandatoryTitle,
					MAWorkbenchMessages.IDEApplication_workspaceMandatoryMessage);
			return EXIT_OK;
		}
		...
		if (instanceLoc.isSet()) {
			...
			try {
				...
				if (workspaceDirectory.exists()) {
					...
					MessageDialog.openError(shell, MAWorkbenchMessages.IDEApplication_workspaceCannotLockTitle,
							MAWorkbenchMessages.IDEApplication_workspaceCannotLockMessage);
				} else {
					MessageDialog.openError(shell, MAWorkbenchMessages.IDEApplication_workspaceCannotBeSetTitle,
							MAWorkbenchMessages.IDEApplication_workspaceCannotBeSetMessage);
				}
			} catch (IOException e) {
				Activator.log("Could not obtain lock for workspace location");
				MessageDialog.openError(shell, MAWorkbenchMessages.InternalError, e.getMessage());
			}
			return EXIT_OK;
		}
		...
		while (true) {
			...
			MessageDialog.openError(shell, MAWorkbenchMessages.IDEApplication_workspaceInUseTitle,
					MAWorkbenchMessages.IDEApplication_workspaceInUseMessage);
		}
	}
	...
	private URL promptForWorkspace(Shell shell, ChooseWorkspaceData launchData, boolean force) {
		...
		do {
			...
			if (instancePath.length() <= 0) {
				MessageDialog.openError(shell, MAWorkbenchMessages.IDEApplication_workspaceEmptyTitle,
						MAWorkbenchMessages.IDEApplication_workspaceEmptyMessage);
				continue;
			}
			...
			try {
				...
				MessageDialog.openError(shell, MAWorkbenchMessages.IDEApplication_workspaceInvalidTitle,
						MAWorkbenchMessages.IDEApplication_workspaceInvalidMessage);
				continue;
			}
	...
	private boolean checkValidWorkspace(Shell shell, URL url) {
		...
		// to use it anyhow.
		String title = MAWorkbenchMessages.IDEApplication_versionTitle;
		String message = NLS.bind(MAWorkbenchMessages.IDEApplication_versionMessage, url.getFile());
	...
	private static String readWorkspaceVersion(URL workspace) {
		...
		try {
			...
			Activator.log("Could not read version file");
			return null;
		}
	}
	...
	private static void writeWorkspaceVersion() {
		...
		try {
			...
			Activator.log("Could not write version file");
		} finally {
			try {
				if (output != null) {
					output.close();
