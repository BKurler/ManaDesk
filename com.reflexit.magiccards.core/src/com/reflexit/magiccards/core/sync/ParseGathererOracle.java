...
public class ParseGathererOracle extends AbstractParseHtmlPage {
	...
	private static Pattern cardnumPattern = Pattern
			.compile("Card Number:\\s*</div>\\s*<div [^<>]*class=\"value\">\\s*(.*?)</div>");
	private static Pattern colorIdPattern = Pattern
			.compile("Color Indicator:\\s*</div>\\s*<div [^<>]*class=\"value\">\\s*(.*?)</div>");
	/*-
	 ...
	 */
	private static Pattern textPattern = Pattern.compile("Card Text:\\s*</div>(.*?)<div class=\"label\"");
	...
	private static Pattern otherSetPattern = Pattern
			.compile("Sets:\\s*</div>\\s*<div class=\"value\">(.*?)<div class=\"label\">");
	private static Pattern expansionPattern = Pattern.compile("Expansion:\\s*</div>.*?set=\\[.*?\\].*?\">(.*?)</a>");
	 ...
	 */
	private static Pattern typesPattern = Pattern.compile("Types:\\s*</div>\\s*<div class=\"value\">\\s*(.*?)</div>");
	/*-
	...
	private static Pattern cardNamePattern = Pattern
			.compile("Card Name:\\s*</div>\\s*<div class=\"value\">\\s*(.*?)</div>");
	/*-
	...
	protected void extractField(MagicCard card, Set<ICardField> fieldMap, String html, MagicCardField field,
			Pattern pattern, boolean multiple) {
		if (fieldMap == null || fieldMap.contains(field)) {
			...
			if (value != null)
				value = value.trim();
			card.set(field, StringCache.intern(value));
		}
	}
	...
	private void applyToDb(ICoreProgressMonitor monitor) {
		...
		if (magicDb != null) {
			for (MagicCard set : sets) {
				IMagicCard card2 = set;
				addNew(card2);
			}
		}
	}
	...
	private boolean someWhatMatches(MagicCard from, MagicCard inet, MagicCard inet2) {
		if (inet == null)
			return false;
		String id = from.getCardId();
		if (!id.equals(inet.getCardId()) && from.getGathererId() != inet.getGathererId())
			return false;
	...
	private boolean adjustUpdateFields() {
		...
		boolean noEnglish = fromCard.getBase().getEnglishCardId() != null;
		if (isOracle()) {
			if (noEnglish) {
				fieldMapFilter.remove(MagicCardField.NAME);
				fieldMapFilter.remove(MagicCardField.TYPE);
	...
	protected void correctSides() {
		...
		if (resultTitle == null || cardA.getName() == null || cardA.getCardId() == null)
			throw new MagicException("Major problems parsing card " + fromCard.getCardId());
		if (cardB == null)
			return;
		if (cardB.getName() == null || cardB.getCardId() == null)
			throw new MagicException("Major problems parsing card " + fromCard.getCardId());
	...
	protected String getUrl() {
		...
		/*
		 * String base = DETAILS_QUERY_URL_BASE + fromCard.getGathererId(); String part
		 * = (String) fromCard.get(MagicCardField.PART); if (part == null) { return
		 * base; } if (part.contains("@")) return base; try { part =
		 * URLEncoder.encode(part, "UTF-8"); } catch (UnsupportedEncodingException e) {
		 * // ignore } return base + "&part=" + part;
		 */
	}
