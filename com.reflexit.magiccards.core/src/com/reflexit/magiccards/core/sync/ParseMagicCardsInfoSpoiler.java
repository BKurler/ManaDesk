...
public class ParseMagicCardsInfoSpoiler extends ParserHtmlHelper {
	...
	public boolean processFromReader(BufferedReader st, ILoadCardHander handler) throws IOException {
		...
		/*
		 * String line = ""; boolean cards = false; String cardtext = ""; boolean in =
		 * false; while ((line = st.readLine()) != null) { if
		 * (countPattern.matcher(line).find()) { Matcher matcher =
		 * countPattern.matcher(line); try { matcher.find(); int c =
		 * Integer.parseInt(matcher.group(1)); handler.setCardCount(c); } catch
		 * (NumberFormatException e) { e.printStackTrace(); } } else if
		 * (cardItemPattern.matcher(line).find()) { cardtext = ""; in = true; continue;
		 * } else if (in && cardItemPatternEnd.matcher(line).find()) {
		 * parseRecord(cardtext, handler); cards = true; in = false; } else if (in) {
		 * cardtext += line; } } if (cards == false) throw new
		 * RuntimeException("No results"); return true;
		 */
	}
	...
	private void parseRecord(String line, ILoadCardHander handler) {
		...
		if (set != null && set.length() > 0) {
			...
			card.setSet(set);
			card.set(MagicCardField.IMAGE_URL, "https://magiccards.info/scans/en/" + abbr + "/" + num + ".jpg");
		}
