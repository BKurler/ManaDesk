...
public class TablePipedImportTest extends AbstarctImportTest {
	...
	public ImportData preimport() {
		try {
			ImportData importData = new ImportData(virtual, deck.getLocation(), line);
			ImportData resultd = ImportUtils.performPreImport(tableImport, importData, ICoreProgressMonitor.NONE);
			result = (List<IMagicCard>) resultd.getList();
			if (resolve) {
				ImportUtils.resolve(resultd.getList());
			}
			setout(result);
			return resultd;
		} catch (Exception e) {
	...
	public void testParts() {
		...
		ImportData im = preimport();
		ArrayList<IMagicCard> mdb = new ArrayList<>();
		ImportUtils.performPreImportWithDb(result, mdb, im.getFields());
